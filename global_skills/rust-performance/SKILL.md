...
6. Hoist allocations out of loops — reuse a single buffer with `.clear()` across iterations instead of allocating a fresh `String`/`Vec` each pass. When rendering a sequence of entries into a single buffer, use `write!(buf, ...)` / `writeln!(buf, ...)` instead of `format!(...)` per entry followed by `buf.push_str(...)` — the `write!` family writes directly into the buffer without creating an intermediate `String`.
...
11. Work directly with the buffer type the library provides rather than converting to `Vec<u8>` and back. If a library returns `Bytes` (as bytes::Bytes, hyper::Bytes, or similar ref-counted zero-copy buffer), pass it through without calling `.to_vec()`. If a function needs `&[u8]`, use `&*bytes` or `bytes.as_ref()` instead of converting to a `Vec`. This avoids an allocation and a copy of the entire buffer.
