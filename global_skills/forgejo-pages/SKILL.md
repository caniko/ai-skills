...
This skill is for Codeberg's shared hosted runners. If the request is for Rust crate CI/release readiness rather than Pages publishing, use the Rust crate release CI skill instead.
...
Use flakes only when the workflow intentionally consumes flake outputs, such as a site/docs build that has no lighter maintained image-based path. Do not use this Pages skill as the source of truth for Rust crate CI; Rust jobs use Debian Rust containers and their own runner setup.
