...
pub(super) fn render_catalog(entries: &[SkillEntry]) -> Result<String> {
    ...
    out.push_str("Generated by `skillctl catalog generate`.\n\n");
...
pub(super) fn matching_entries<'a>(entries: &'a [SkillEntry], skill: &str) -> Vec<&'a SkillEntry> {
    entries
        .iter()
        .filter(|entry| entry.name == skill || entry.qualified_name == skill)
        .collect()
}

pub(super) fn print_entry(entry: &SkillEntry) {
    println!("name: {}", entry.name);
    println!("qualified_name: {}", entry.qualified_name);
