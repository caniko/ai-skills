...
description: Create or uplift Rust project Nix flakes using crane. Use when Codex needs to add, generate, repair, or modernize flake.nix, devShells, checks, packages, cross-compilation, Cargo source filtering, or Nix build infrastructure for a Rust crate or workspace. Always use crane for Rust builds; prefer rs-harbor helpers when available or acceptable.
---
...
3. Implement narrowly:
   ...
   - Add small `nix/*.nix` files only when they reduce clutter or match existing project style.
