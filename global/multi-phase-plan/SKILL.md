...
| `Recommended planner flavour` | Must match the currently invoked flavour (`multi-phase-plan-codex`, `multi-phase-plan-claude`, `multi-phase-plan-mixed`, or `multi-phase-plan-opencode`). If it names a different flavour, stop and ask the user whether to switch flavour or intentionally override the dossier recommendation. |
