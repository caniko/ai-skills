...
5. Check for similar existing PRs before opening a new one.
   Search `NixOS/nixpkgs` for open PRs and recently closed PRs that touch the same package, module, service, or version bump.
   Use both the package or module name and a likely title fragment such as `pname: old -> new`.
   A typical search sequence is:

   ```bash
   gh search prs --repo NixOS/nixpkgs --state open "pname"
   gh search prs --repo NixOS/nixpkgs --state merged "pname"
   gh search prs --repo NixOS/nixpkgs --state closed "pname"
   ```

   Read the most relevant hits before publishing.
   If a substantially similar PR already exists, stop and report it to the user instead of opening a duplicate unless the user explicitly wants a replacement or follow-up PR.
...
7. Review the PR with `caniko/nixpkgs-review-gha`.
