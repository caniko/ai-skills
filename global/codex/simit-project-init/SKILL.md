...
- After a successful patch, modularize only when it reduces real complexity or when `flake.nix` is large enough that future simit or project edits would become hard to review.

Use the shared modularization reference instead of inventing a different split:

```text
/home/can/.codex/skills/simit-project-init/references/flake-modularization.md
```
...
If `simit init-ci --check` fails, regenerate with the same options used by the existing workflow, then inspect the diff before reporting done.
